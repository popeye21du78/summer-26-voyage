# Prompt API — Fiche patrimoine (chapelles, citadelles, ponts, sites divers)
...
- Éviter : dates en accumulation, démonstrations savantes.
...
L'esthétique est centrale. Décrire lumière, matières, couleurs, volumes, perspectives. Descriptions d'ambiance **léchées** : soignées, évocatrices, sans remplissage. Toute phrase d'ambiance s'appuie sur un élément concret.
...
**Les verbes de 3e personne ne prennent JAMAIS d'accolades.** Quand le sujet est le site, la chapelle, le pont, la pierre, « on », une relative, etc., écrire le verbe normalement. Seuls les verbes où **le lecteur (tu/vous) est le sujet grammatical** prennent des accolades.

Exemples corrects : `{tu pourras,vous pourrez}`, `{ton,votre}`, `{Commence,Commencez}`, `{Lève,Levez} les yeux`.
...
**Exception** : les sections BONUS n'utilisent **pas** les accolades. BONUS_SEUL est écrit en « tu » directement. BONUS_COUPLE, BONUS_FAMILLE et BONUS_AMIS sont écrits **entièrement** en « vous » directement — y compris les **possessifs**. Ne pas mélanger « vous » et possessifs « tu ».

### 2.2 — Crochets `[ ]` — accord genre

Toujours **2 formes** `[masc,fem]`. Le code choisit selon le genre du lecteur.

- **Sections principales** et **BONUS_SEUL** : au **singulier**. Ex. `[ravi,ravie]`.
- **BONUS_COUPLE / FAMILLE / AMIS** : au **pluriel**. Ex. `[ravis,ravies]`.

Règles :
- Sans espaces après la virgule.
- Pas de crochets pour les invariables.
...
- En cas de doute sur un détail, ne pas le mentionner.
- Les dates, commanditaires et fonctions doivent être exacts.

---

## Partie 4 — Structure et consignes

Tu génères **une fiche patrimoine** pour un site historique (chapelle, citadelle, pont, fortification, site archéologique…). Structure allégée, avec les **4 BONUS obligatoires**.
...
- **1ère phrase OBLIGATOIRE** : `[Cher,Chère] [PRENOM], bienvenue à/au [NOM] :` suivi du type de site, de l'époque et du cadre géographique. Adapter la préposition (à la, au, aux, devant…).
- Architecture, atmosphère, première impression.
- ≥ 2 ancrages concrets.
- `[PRENOM]` : exactement **1×**.
- Accolades `{}` pour chaque verbe/pronom de 2e personne.

**HISTOIRE** (60–120 mots, 2 paragraphes) :
- Origine, fonction passée, transformations, ce qu'il en reste.
- **Aucune** accolade, **aucun** crochet, **aucun** placeholder, **aucun** pronom de 2e personne.

**QUE_VOIR** (60–120 mots, 2 paragraphes) :
- Détails à observer, singularités du lieu, éléments remarquables.
...
- « à couper le souffle », « joyau », « chef-d'œuvre architectural », « incontournable », « ne pas manquer », « plongée fascinante », « hors du temps », « suspendre le temps », « valeur sûre », « havre de paix », « loin du tumulte ».
