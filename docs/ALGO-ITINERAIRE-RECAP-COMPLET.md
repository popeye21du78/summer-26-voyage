...
| **Génération** | ✅ Aligné récap v2 | `lib/itinerary/generate.ts` — corridor global S→E + top-N, greedy clusters adaptatifs, épine multi-start avec end dérivé, budget par segment, insertion gloutonne (corridor par segment d'épine), PAS de 2-opt global, nuits avec temps de conduite + targetKmPerDay |
...
| Ratio additif | ✅ Implémenté | `lib/itinerary/greedy-insert.ts` — `score / (détour×terrain + α×perp + γ×angle)` |
| sqrt(densité) zone×catégorie | ✅ Implémenté | `lib/itinerary/zone-matrix.ts` |
| Greedy clustering (vs DBSCAN) | ✅ Implémenté | `lib/itinerary/greedy-cluster.ts` (DBSCAN conservé dans le repo mais non utilisé par défaut) |
| Soft quotas (bonus) | ✅ Implémenté | `greedy-insert.ts` — bonus si catégorie sous-représentée |
| Diversity penalty | ✅ Implémenté | `greedy-insert.ts` — pénalité si même `bucketFamille` consécutif |
| target_km_per_day (nuits) | ✅ Implémenté | `nights.ts` + `generate.ts` (`targetKmPerDay`) |
| Multi-start spine | ✅ Implémenté | `lib/itinerary/spine.ts` — projection, TSP, shuffles, filtre angles |
| Recuit simulé final | ⏳ Optionnel | Non implémenté (voir §12.9) |
| Facteur terrain détour | ✅ Implémenté | `lib/itinerary/terrain.ts` |
| 2-opt ouvert | ✅ Dispo (non global) | `lib/itinerary/tsp.ts` — `twoOptReorder` disponible mais **non appliqué globalement** (détruisait l'ordre de l'épine) |
| **Corridor global S→E pre-filter** | ✅ Implémenté v2 | `generate.ts` — filtre corridor + top-N avant clustering |
| **Corridor par segment d'épine** | ✅ Implémenté v2 | `greedy-insert.ts` — `inSegmentCorridor` vérifié sur le segment d'épine (pas la micro-arête) |
| **Budget de visites** | ✅ Implémenté v2 | `generate.ts` — `targetVisits = routeDays × visitsPerDay(rythme)` |
| **Nb clusters adaptatif** | ✅ Implémenté v2 | `generate.ts` — `maxClusters = min(8, max(2, ceil(routeDays/1.5)))` |
| **Dérivation end** | ✅ Implémenté v2 | `generate.ts` — `deriveFarthestEnd` si pas de point d'arrivée explicite |
| **Temps conduite nuits** | ✅ Implémenté v2 | `nights.ts` — budget journalier inclut conduite + visite adaptée au type |
| **Max détour par insertion** | ✅ Implémenté v2 | `greedy-insert.ts` — `min(60km, 35% budget segment)` par point |
