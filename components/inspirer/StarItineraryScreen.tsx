...
export default function StarItineraryScreen({ slug }: Props) {
  ...
  return (
    <main className="flex h-full flex-col bg-[var(--cream)]">
      {/* Top bar */}
      <div className="flex shrink-0 items-center justify-between border-b border-[var(--color-accent-start)]/15 bg-[var(--cream)] px-4 py-3">
        <Link
          href={backHref}
        ...
        <button
          type="button"
          className="flex items-center gap-1 rounded-full border border-[var(--color-accent-start)]/25 px-3 py-1.5 font-courier text-[10px] font-bold text-[var(--color-accent-start)] transition hover:border-[var(--color-accent-start)]/40"
        >
      ...
      >
        {coverSrc ? (
          ...
          <div className="flex h-full items-center justify-center bg-gradient-to-br from-[var(--color-bg-main)] to-[var(--color-accent-start)]/20">
            <MapPin className="h-12 w-12 text-[var(--color-accent-start)]/30" />
          </div>
        )}
      </div>
      ...
      <button
        ...
        className="flex shrink-0 items-center justify-center gap-1 border-b border-[var(--color-accent-start)]/10 bg-[var(--cream)] py-2 font-courier text-[10px] font-bold uppercase tracking-wider text-[var(--color-accent-start)]/60"
      >
        <ChevronDown
      ...
      <div ref={contentRef} className="min-h-0 flex-1 overflow-y-auto px-5 py-6">
            ...
            <div className="mt-4 space-y-3">
              {steps.map((step, i) => (
                <Link
                  ...
                  className="flex items-center gap-3 rounded-xl border border-[var(--color-accent-start)]/20 bg-white p-3.5 transition hover:border-[var(--color-accent-start)]/40"
                >
                  <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[var(--color-accent-start)]/15 font-courier text-xs font-bold text-[var(--color-accent-start)]">
                    {i + 1}
                  </span>
        ...
        {editorial?.suggestedPoiAdditions && editorial.suggestedPoiAdditions.length > 0 && (
          <section className="mt-8 rounded-2xl border border-[var(--color-accent-start)]/15 bg-[#FFF8F2] p-5">
            ...
            <div className="mt-3 space-y-2">
              {editorial.suggestedPoiAdditions.map((poi, i) => (
                <div key={i} className="flex items-start gap-2">
                  <Sparkles className="mt-0.5 h-3.5 w-3.5 shrink-0 text-[var(--color-accent-start)]/60" />
                  <div>
        ...
        <div className="mt-10 space-y-3 pb-4">
          <Link
            ...
            className="flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-[var(--color-accent-start)] to-[var(--color-accent-end)] py-4 font-courier text-sm font-bold text-white shadow-[0_10px_36px_rgba(224,120,86,0.45)] transition hover:brightness-105"
          >
          ...
          <button
            type="button"
            className="flex w-full items-center justify-center gap-2 rounded-2xl border-2 border-[var(--color-accent-start)]/30 py-3 font-courier text-sm font-bold text-[var(--color-accent-start)] transition hover:border-[var(--color-accent-start)]/50"
          >
