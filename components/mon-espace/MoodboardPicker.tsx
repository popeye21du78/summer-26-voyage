  ...
  return (
    <section className="viago-glass-card px-4 py-4">
      ...
      <p className="mb-3 font-courier text-xs leading-relaxed text-[var(--color-text-secondary)]">
        Test rapide de 5 paires modernes (contenu + titres), comme pour le moodboard.
      </p>
      <ul
        role="radiogroup"
        ...
        {FONT_PRESETS.map((f) => {
          ...
          return (
            <li key={f.id}>
              ...
              >
                <p className="font-courier text-[11px] font-bold uppercase tracking-wider text-[var(--color-text-primary)]">
                  {f.label}
                </p>
                <p className="mt-1 font-courier text-[10px] text-[var(--color-text-secondary)]">
                  Contenu: {f.contentLabel} · Titres: {f.titleLabel}
                </p>
              </button>
            </li>
          );
