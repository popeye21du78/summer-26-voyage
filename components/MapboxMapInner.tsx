...
function getMarkerStyle(
  ...
  switch (nuiteeType) {
    case "airbnb":
      return { size, color: "var(--color-accent-start)" };
    case "van":
      return { size, color: "#a8987a" };
    case "passage":
      return { size: 10, color: "#c4b89a" };
    default:
      return { size: 10, color: "var(--color-accent-start)" };
  }
}
...
function CityPopupOverlay({
  ...
  return (
    <div
      ref={popupRef}
      className="absolute bottom-2 left-1/2 z-10 w-[220px] -translate-x-1/2 rounded border-2 border-[var(--color-accent-start)] bg-[#FAF4F0] p-2 shadow-lg"
    >
      <button
        type="button"
        onClick={onClose}
        className="absolute right-1 top-1 rounded p-0.5 text-[#6b6b6b] hover:bg-[var(--color-accent-start)]/20"
        aria-label="Fermer"
      >
        ×
      </button>
      <div className="flex flex-col gap-2">
        <div className="relative h-16 w-full shrink-0 overflow-hidden rounded bg-[var(--color-accent-start)]/15">
          <CityPhoto
            stepId={step.id}
            ville={step.nom}
            alt={step.nom}
        ...
        <div className="flex shrink-0 flex-col gap-1">
          <Link
            ...
            className="block rounded bg-[var(--color-accent-start)] px-2 py-1.5 text-center text-[10px] font-medium text-white no-underline transition-colors hover:bg-[var(--color-accent-start)]"
          >
            Infos ville
          </Link>
          <Link
            href={`/book#${step.id}`}
            className="block rounded border border-[var(--color-accent-start)] px-2 py-1.5 text-center text-[10px] font-medium text-[var(--color-accent-start)] no-underline transition-colors hover:bg-[var(--color-accent-start)]/10"
          >
            Voir dans le Book
          </Link>
          <Link
          ...
          >
            <div
              ...
              style={{
                ...
                backgroundColor: selectedStep?.id === step.id ? "var(--color-accent-start)" : color,
              }}
            >
              {nuitees > 0 && (
                <span
            ...
            {hoveredStep?.id === step.id && (
              <span className="absolute left-full top-1/2 z-10 ml-2 -translate-y-1/2 whitespace-nowrap rounded-full bg-white/95 px-2 py-0.5 text-[10px] font-bold text-[#333333] shadow-sm ring-1 ring-[var(--color-accent-start)/30]/50">
